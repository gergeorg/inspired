			...
			<div className={style.body} onClick={handleModalClick}>
				...
				<ul className={style.customer}>
					...
					<li className={style.customerItem}>
						...
						<span className={style.customerData}>{values.delivery === 'delivery' ? 'Доставка' : 'Самовывоз'}</span>
					</li>
				</ul>
					...
					{order.map((item) => {
						...
						return (
							<li className={style.goodsItem} key={`${item.id}${item.color}${item.size}`}>
								<img alt={product.title} className={style.goodsImg} src={`${API_URL}/${product.pic}`} />
				...
				<button aria-label='Закрыть модальное окно' className={style.close} type='button' onClick={handleCloseModal} />
			</div>
		</div>
	)
}
